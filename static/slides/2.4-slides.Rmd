...
- .green[Investment] $i_t$ increases

- Steady state level of capital $k_t^*$ increases

- Steady state .blue[output] increases $y_t^*$

- Steady state amount of .purple[consumption]
...
- What if .red[depreciation] costs increase?
    - $\delta_1=0.02$
    - $\delta_2=0.04$
...
- .green[Investment] $i_t$ decreases

- Steady state level of capital $k_t^*$ decreases

- Steady state .blue[output] decreases $y_t^*$

- Steady state amount of .purple[consumption] $c_t^*$ decreases 
]
...
.smallest[
- Different values of $s$ lead to different steady state levels of $k^*$, so **which is _best_?**

- The best steady state is one where there is the highest possible .purple[consumption] per person 
$$c^*=(1-s)f(k^*)$$
...
- **Find the value of $s$ (and $k^*)$ that _maximizes_ $c^*$**
]
]
...
- .hi[Golden Rule level of] $\color{#e64173}{\mathbf{k^*_{GR}}}$ where slope of depreciation line $=$ slope of production function, $f(k^*)$
...
- .hi[Golden Rule level of] $\color{#e64173}{\mathbf{k^*_{GR}}} = \frac{\delta k^*_{GR}}{y^*_{GR}}$
...
- Policy implications: policymakers can choose $s$ to maximize $c_i^*$ at $k^*_{GR}$
...
- .hi-purple[Population] grows at constant rate $n$ over time
  - $L_{t+1}=L_t(1+n)$
  - $g_L = n$

- .hi-purple[Technology] grows at constant rate $g$ over time
  - $A_{t+1}=A_t(1+g)$
...
| **Output per worker** | $\mathbf{\frac{Y}{L}=Ay}$ | $\mathbf{g}$ |
| TFP | $A$ | $g$ |
...
.smallest[
- Interesting: .hi-turquoise[growth rate of output per worker grows *solely* from rate of TFP progress `\\((g)\\)`!]
]
]
...
- In other words - .hi-turquoise[the further away from (closer to) $k^*$ your country is, the faster (slower) you should grow]
]
...
ggplot(data = solow)+
  ...
  labs(x = "Time",
       y = "Growth Rates (%)",
       title = "Growth Accounting for U.S., 1950-2010",
...
ggplot(data = solow)+
  ...
  geom_hline(yintercept = 0, color = "black", size = 1)+
  geom_line(aes(y = A_change, color="Technology (A)"),  size = 2)+
  geom_hline(aes(yintercept=mean(A_change,na.rm=T)), color = "black", linetype="dashed", size = 1)+
  geom_text(aes(x=1980, y=0.5+mean(A_change,na.rm=T)), label="Average TFP Growth: 1.373%", size = 5, color = "black")+
  scale_x_continuous(breaks=seq(1950,2010,10),
                     limits=c(1948,2012))+
  #scale_y_continuous(breaks=seq(0,10,1),
  #                   limits=c(0,10),
...
.source[Source: [Federal Reserve Bank of San Francisco](https://www.frbsf.org/economic-research/indicators-data/total-factor-productivity-tfp/)]
...
.center[
![:scale 65%](../images/tfp-trend.png)
]

.source[Sources: [Eli Dourado](https://elidourado.com/tfp/); [Federal Reserve Bank of San Francisco](https://www.frbsf.org/economic-research/indicators-data/total-factor-productivity-tfp/)]