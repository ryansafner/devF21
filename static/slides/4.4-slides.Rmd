---
title: "4.4 — The Selectorate Model"
...
- .hi[Autocracy] is an all-encompassing term for any non-democratic/constitutional republic political system
...
.pull-left[
.smaller[
- .hi-purple[Every leader's goal is to attain and maintain political power (get and stay in office)]
...
- The variation around the world is in the .hi-turquoise[institutions regarding how the leader is selected]
]
]
...
- .hi[Selectorate theory]: characterizes governments by their selection institutions
...
.source[Bueno de Mesquita, Bruce, Alastair Smith, Randolph Siverson, and James Morrow, 2006, *The Logic of Political Survival*]
]
...
2. .hi[Selectorate `\\((S)\\)`]: subset of residents who can potentially select the Leader
...
.center[
![:scale 80%](https://www.dropbox.com/s/37xrmhiggtos4ds/selectorate.png?raw=1)
...
- Each society has a **Leader** and a set of potential substitute leader(s), the .hi-purple[Challenger `\\((C)\\)`] 
...
1. .hi-purple[Private Goods (rents)] given exclusively **to members of the winning coalition** $(W)$
  - money, tax breaks, management of state-owned enterprises, political positions, prestige

2. .hi-purple[Public Goods] made available for **all** Residents
...
- .hi-turquoise[Challenger (C)] makes an offer (tax rate, private goods, public goods)

- Whomever makes a better offer to the Selectorate is selected as Leader

- .hi-purple[Leader (L)] must keep enough coalition members happy to avoid them defecting
]
...
- But challenger faces a .hi[commitment problem]
...
1. .hi-purple[The size of the winning coalition, `\\(W\\)`]

2. .hi-purple[The loyalty norm, `\\(\frac{W}{S}\\)`]
...
- .hi-purple[The probability of being in a leader's winning coalition is `\\(\frac{W}{S}\\)`]
...
- So a potential defector in $W$ faces a tradeoff:
  - (Expected) .hi-green[marginal benefit of defecting]: being in $W$ under $C$
  - (Expected) .hi-red[marginal cost of defecting]: in $W$ now under $L$, but not being in $W$ under $C$
...
- As $W \uparrow$ or $\downarrow S$, $C$ is less likely to need any particular member $W_i$ (and less likely to include them in new regime)
  - Defecting becomes riskier<sup>.magenta[†]</sup>

- Risk of *not* being in $W$ is $(1-\frac{W}{S})$
]
]

.footnote[<sup>.magenta[†]</sup> Not to mention, if the coup fails, or your defection is discovered beforehand, Leaders are often unkind to traitors!]
...
.pull-right[
.center[
![](../images/portraitofleaderstalin.jpg)
]
]
---
...
- Leader of Society A must spend $10,000 on each `\\(W_i\\)` member vs. $100,000 in Society B
]
...
.pull-left[
- In high loyalty (small $\frac{W}{S})$ societies, leaders need to spend *less* to keep $W$ happy, meaning more tax revenue leftover for .hi-purple[discretionary spending] by the leader
...
- Discretion also creates possibility of .hi-purple[kleptocracy] - not mere corruption (buying off members of coalition for support), but *outright theft of state resources*
...
- .hi-purple[Rents are the glue that hold the dominant/winning coalition together]
